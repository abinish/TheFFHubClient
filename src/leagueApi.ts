...
export async function getLeagueHistory( parameters: {
    site: string,
    leagueId: string,
    userId: string,
    swid: string,
    ...
    const response = await fetch(`https://api.theffhub.com/api/leagueData/leagueHistory?site=${parameters.site}&leagueId=${parameters.leagueId}&userId=${parameters.userId}&swid=${parameters.swid}&s2=${parameters.s2}`)
    checkStatus(response);
    return await response.json() as ILeagueDetails[];
